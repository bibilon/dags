...
def get_dynamic_dag_names():
    ...
    jdbc_driver = Variable.get("jdbc_driver")
    jdbc_driver_loc = Variable.get("jdbc_driver_loc")
    db_url = Variable.get("db_url")
    user = Variable.get("user")
    password = Variable.get("pass")

    # Kết nối với cơ sở dữ liệu Oracle thông qua JDBC
    connection = jaydebeapi.connect(jdbc_driver, db_url, {'user': user, 'password': password}, jars=jdbc_driver_loc)
